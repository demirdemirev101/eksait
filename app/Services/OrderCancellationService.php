...
class OrderCancellationService
{
    ...
    {
        $order->loadMissing(['shipment', 'items.product', 'items.variant']);
        ...
        try {
            ...
            if ($labelWasCancelled && $shipment) {
                $this->shipmentCancellationService->clearCancelledShipmentData($shipment, [
                    'error_message' => 'Carrier label was cancelled, but order cancellation did not finish: '.$e->getMessage(),
                ]);
            }
        ...
        DB::transaction(function () use ($order): void {
            $lockedOrder = Order::whereKey($order->id)
                ->lockForUpdate()
                ->firstOrFail();
            $lockedOrder->load(['shipment', 'items.product', 'items.variant']);
