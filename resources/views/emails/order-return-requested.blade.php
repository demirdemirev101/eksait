...
@section('title', __('orders.mail.return_requested.title'))

@section('hero')
    <strong>{{ __('orders.mail.return_requested.hero_title', ['name' => $order->customer_name]) }}</strong><br>
    {{ __('orders.mail.return_requested.hero_text', ['order' => $order->id]) }}
@endsection
...
@section('content')
    ...
    <p style="margin: 0; color: #555555; font-size: 14px; line-height: 1.7;">
        {{ __('orders.mail.return_requested.support') }}
    </p>
@endsection
